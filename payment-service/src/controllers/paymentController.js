...
async function internalCharge(req, res) {
    try {
        const { amount, currency, metadata } = req.body;
        const result = await paymentService.charge({ amount, currency, metadata });
        return res.status(200).json(result);
    } catch (error) {
        return res.status(400).json({ message: 'Failed to process internal charge', error: error.message });
    }
}
